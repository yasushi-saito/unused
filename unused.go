...
func (c *Checker) findExportedInterfaces() {
	c.interfaces = []*types.Interface{
		types.Universe.Lookup("error").Type().(*types.Named).Underlying().(*types.Interface),
		types.Universe.Lookup("sort").Type().(*types.Named).Underlying().(*types.Interface),
	}
...
func (c *Checker) visit(pass *analysis.Pass) (interface{}, error) {
	log.Printf("Package %s start", pass.Pkg.Path())
	c.mu.Lock()
	...
	c.processAST(pass)
	c.mu.Unlock()
	log.Printf("Package %s done", pass.Pkg.Path())
	return nil, nil
}
